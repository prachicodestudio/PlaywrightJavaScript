...
- Name: temp.spec.js >> Playwright Title Verification
- Location: tests\temp.spec.js:22:5
...
```ts
  ...
  2  | 
  3  | // Test 1 - Always Pass
  4  | test('SauceDemo Login', async ({ page }) => {
  5  | 
  6  |     // Open SauceDemo website
  7  | 
  8  |     await page.goto('https://www.saucedemo.com/');
  9  | 
  10 |     // Login
  11 |     await page.fill('#user-name', 'standard_user');
  12 |     await page.fill('#password', 'secret_sauce');
  ...
  19 | 
  20 | 
  21 | // Test 2 - Always Fail
  22 | test('Playwright Title Verification', async ({ page }) => {
  23 | 
  24 |     // Open Playwright website
  25 |     await page.goto('https://playwright.dev/');
  26 | 
  27 |     // Intentionally wrong title
> 28 |     await expect(page).toHaveTitle('Wrong Title');
     |                        ^ Error: expect(page).toHaveTitle(expected) failed
  29 | 
  30 | });
  31 | 
  32 | 
  33 | // Test 3 - Flaky Test
  34 | test('Flaky Demo', async ({ page }, testInfo) => {
  35 | 
  36 |     // Open any website
  37 |     await page.goto('https://playwright.dev/');
  38 | 
  39 |     // Fail on first attempt
  40 |     if (testInfo.retry === 0) {
  41 | 
  42 |         console.log('First Attempt - Failed');
  43 | 
  ...
  47 | 
  48 |     // Pass on retry
  49 |     console.log('Retry Attempt - Passed');
  50 | 
