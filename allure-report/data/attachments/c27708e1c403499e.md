...
- Name: AllureReport.spec.js >> Playwright has title
- Location: tests\AllureReport.spec.js:10:5
...
Expected: "123Fast and reliable end-to-end testing for modern web apps | Playwright"
...
```ts
  ...
  2  | test('SauceDemo Login', async({page})=>{
  3  |    //open url
  ...
  8  | })
  9  | 
  10 | test('Playwright has title', async ({ page }) => {
  11 |   await page.goto('https://playwright.dev/');
  12 | 
> 13 |   await expect(page).toHaveTitle("123Fast and reliable end-to-end testing for modern web apps | Playwright");
     |                      ^ Error: expect(page).toHaveTitle(expected) failed
  14 | });
  15 | 
  16 | test('Facebook has title', async ({ page }) => {
  17 |   await page.goto('https://facebook.com/');
  18 | 
  19 |   // Expect a title should be Facebook
